...
Run Better Auth without requiring an HTTP-facing `baseURL`.

Use this when the auth instance is used from trusted server-side code, such as workers, queues, scheduled jobs, or internal services, and you do not mount the Better Auth HTTP handler.
...
Headless mode does not create an ambient current user. Request-scoped APIs still require request headers or an explicit session token. For trusted server-side user work, use the server API:

```ts
const accounts = await auth.server.getUser(userId).listAccounts();
const session = await auth.server.getUser(userId).createSession();
const resolved = await auth.server.session(session.token).get();
```

Plugins can expose their own trusted server APIs. For example, when the admin plugin is configured:
...
const { user } = await auth.server.admin.createUser({
	email: "user@example.com",
	name: "User",
})

await auth.server.admin.setRole({
	userId: user.id,
	role: "admin",
})
```

URL-generating flows such as OAuth callbacks, magic links, email verification, and password reset links still need a public URL when used.
